...
> **硬件分配（详见 §8.1）**：预验证轮全程本地 PRO6000；正式首轮起**深度（含 Adapter）留本地 PRO6000，检测/分割/房间上云并行**——云上并行摆脱单卡串行瓶颈。
...
```text
数据: ScanNet / HM3D / Matterport3D / 合成数据，15–25 类房间
...
预验证轮全程本地；正式首轮起按 head 分流——**深度（含 Adapter）留本地，检测/分割/房间上云并行**：

| 轮次 / 阶段 | 硬件 | 说明 |
|---|---|---|
| 预验证轮（L0/L1/L2） | 本地 1×PRO6000(96GB) | 全部 head 在本地验证、打磨框架 |
| 正式首轮·阶段0 Adapter+Depth | 本地 1×PRO6000(96GB) | **必须本地**：Adapter 在此训练并冻结（其余 head 的前置）；深度数据含本地双目 teacher，96GB 承载阶段0 峰值 |
| 正式首轮·阶段1/2/3 Det/Mask/Room | 云平台（多卡） | Adapter 冻结后各 head 独立 → 云上**并行**训练，摆脱单卡串行瓶颈 |
| 完整数据轮 | 同上分工 | 深度本地、其余上云 |

**本地 → 云 交接（关键）：**

- 阶段0 完成后导出**冻结特征基座**（DINOv2 + Adapter）权重（~0.2–0.3 GB），连同检测/分割/房间数据集上云；
- ⚠️ **版本钉死**：云端各 head 必须基于与本地**逐字节一致**的冻结基座训练（ckpt hash 比对）——否则 head 学到的特征 ≠ 部署时特征，**无损合并失效**（见 §1 铁律 / §11）；
- 云端两种喂数方式：① 直接跑 DINOv2+Adapter forward；② 预缓存 Adapter 特征(§8.3)再训 head（省算力但特征体积大，见 §7.1）。

**收益：** 检测/分割/房间在云上并行，正式首轮总周期由"各阶段相加" → **阶段0(本地) + max(阶段1,2,3)(云)**，并可在云端多卡 DDP 进一步压缩检测耗时。
...
| **合计（本地深度 + 云并行）** | | **~5 天** | | **~50 天（≈7 周）** |
| 首轮（阶段0+1） | | **~5 天** | | **~50 天（≈7 周）** |

> 口径：阶段0 本地 PRO6000；阶段1/2/3 上云**并行**（§8.1），故合计 ≈ **阶段0 + max(阶段1,2,3)**，而非各阶段相加。
> 深度时间已按 **896×504（吞吐 ~40 img/s）** 更新（比 518 主力约 ×1.5）。
> 含 ×1.5–2 wall-clock 余量后：**最低数据量 ~1.5 周；完整数据量 ~10–14 周**。

观察：

- **最低数据量首轮约一周出结果**（含调试 ~1.5 周），仍适合快速验证统一框架链路；
- **瓶颈几乎全在阶段0 深度**（本地、串行、896×504 稠密）——完整数据量下 ~43 天，占绝对大头；
- 阶段1/2/3 上云并行后只受**最慢的检测**约束（完整 ~7 天），云端多卡 DDP 还能再压；表内各 head 时间为"单卡在线"上限，云上更快。

### 10.3 里程碑（最低/原型数据量；深度本地 + 其余云并行）
